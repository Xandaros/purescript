...
inferForeignModules = fmap (M.mapMaybe id) . traverse inferForeignModule
  where
    ...
    inferForeignModule (Right path) = do
      let jsFile = replaceExtension path "lua"
...
buildMakeActions outputDir filePathMap foreigns usePrefix =
  ...
  codegen m _ exts = do
    ...
    foreignInclude <- case mn `M.lookup` foreigns of
      Just path
        ...
        | otherwise -> do
            ...
            return $ Just $ J.JSApp Nothing (J.JSVar Nothing "require") [J.JSStringLiteral Nothing (filePath ++ "./foreign")]
    ...
    let jsFile = outputDir </> filePath </> "init.lua"
        mapFile = outputDir </> filePath </> "init.lua.map"
        externsFile = outputDir </> filePath </> "externs.json"
        foreignFile = outputDir </> filePath </> "foreign.lua"
  ...
  genSourceMap dir mapFile extraLines mappings = do
    let pathToDir = iterate (".." </>) ".." !! length (splitPath $ normalise outputDir)
        sourceFile = case mappings of
                      ...
                      _ -> Nothing
    let rawMapping = SourceMapping { smFile = "init.lua", smSourceRoot = Nothing, smMappings =
